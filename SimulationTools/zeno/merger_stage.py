...
parser = argparse.ArgumentParser(description="Determine a numerical merger \
                                              stage based on an input nuclear \
                                              separation curve.")
parser.add_argument('center', type=str, help='Centers file')
parser.add_argument('-tstage', action='store_true', default=True,
                    help='Compute the merger stage between successive passes \
                          linear in time.')
...
parser.add_argument('--eps', action='store', type=float, default=0.,
                    help='Smoothing length for the simulation (if given, will \
                          clip minima below that)')
parser.add_argument('--tnow', action='store', type=float, default=0.,
                    help='Viewing time.')
parser.add_argument('--minorder', action='store', type=int, default=10,
                    help='Number of points on either side of minima for it to \
                          be considered valid. (Default:10)')
parser.add_argument('--tscl', action='store', type=float, default=1.0,
                    help='Time scaling factor (Myr)')
parser.add_argument('--Lscl', action='store', type=float, default=1.0,
                    help='Length scaling factor (kpc)')
parser.add_argument('--savefig', action='store', type=str, default=False,
                    help='Save figure of the nuclear separation for \
                          diagnostics.')
